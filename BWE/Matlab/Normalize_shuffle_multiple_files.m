...
for i=1:length(file_list_feature)%feature
    
    load([input_location_feature '/' sprintf('train1_feature_%d', i)]);
    %load([input_location_feature '/' sprintf('train2_feature_%d', i)]);
    
    target_all = ['target = target_', num2str(i), ';'];
    eval(target_all);
    target_new = ['target_', num2str(i), '=zeros(size(target,1),4);'];
    eval(target_new);
    
    for j=1:4
        target_mean = ['target_', num2str(i), '(:,j)=mean(target(:,1+4*(j-1):4*j),2);'];
        eval(target_mean);
    end
    
    save([output_location_feature sprintf('train1_feature_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
    
end



for i=1:length(file_list_target) %target 
    ...
    for j=1:4
     ...
     save([output_location_target '/' sprintf('test_feature_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
   
end

% Calculate mean & standard deviation parameters

train_len=zeros(length(file_list_feature),1);
test_len=zeros(length(file_list_target),1);
...
for i=1:length(file_list_feature) %feature
    ...
    mean_f_save=['mean_f_', num2str(i), '=mean(input_', num2str(i), ');'];
    eval(mean_f_save);
    var_f_save=['var_f_', num2str(i), '=var(input_', num2str(i), ');'];
    eval(var_f_save);
    ...
    len_save=['train_len(', num2str(i), ',1)=length(input_', num2str(i), ');'];
    eval(len_save);
    
    clear(sprintf('input_%d', i), sprintf('target_%d', i));
...
for i=1:length(file_list_feature)
...
%save 'MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET1/normal_norm_f' normal_mean_f normal_std_f
%save 'MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/normal_norm_f' normal_mean_f normal_std_f
%save MATLAB/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/normal_norm_f normal_mean_f normal_std_f;
%save 15_16/original_2/normal_norm_f normal_mean_f normal_std_f;
...
load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET1/normal_norm_t
%load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/train_len
%load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/normal_norm_f
%load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/normal_norm_t
...
for i=1:length(file_list_feature)
    ...
    buf = ['buf1 = bsxfun(@minus, input_', num2str(i), ', normal_mean_f);'];
    eval(buf);
    input = ['input_', num2str(i), '= bsxfun(@rdivide, buf1, normal_std_f);'];
    eval(input);
    
    buf = ['buf2 = bsxfun(@minus, target_' num2str(i), ', normal_mean_t);'];
    eval(buf);
    target = ['target_', num2str(i), '= bsxfun(@rdivide, buf2, normal_std_t);'];
    eval(target);
    
    save([output_location_feature sprintf('train1_1regression1_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
    %save([output_location_feature sprintf('train2_1regression1_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
    clear(sprintf('input_%d', i), sprintf('target_%d', i));
end
...
load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET1/normal_norm_f
load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET1/normal_norm_t
%load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/train1_len
%load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/normal_norm_f
%load MATLAB/FFT64/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/normal_norm_t
...
for i=1:length(18)
    ...
    len_save=['test_len(', num2str(i), ',1)=length(input_', num2str(i), ');'];
    eval(len_save);
    
    save([output_location_target '/' sprintf('test_1regression1_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
    clear(sprintf('input_%d', i), sprintf('target_%d', i));
end
...
for i=1:40
    ...
    for j=1:40
        if(i~=j)
            ...
            input_all = ['input = [input_', num2str(i), ';input_', num2str(j), '];'];
            eval(input_all);
            ...
            input = input(randomorder(1:totnum), :);
            target = target(randomorder(1:totnum), :);
            
            input_all = ['input_', num2str(i), '= input(1:train_len(i),:);'];
            eval(input_all);
            target_all = ['target_', num2str(i), '= target(1:train_len(i),:);'];
            eval(target_all);
            input_all = ['input_', num2str(j), '= input(1+train_len(i):end,:);'];
            eval(input_all);
            target_all = ['target_', num2str(j), '= target(1+train_len(i):end,:);'];
            eval(target_all);
            
            save([output_location_feature sprintf('train1_1regression2_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
            save([output_location_feature sprintf('train1_1regression2_%d', j)], sprintf('input_%d', j), sprintf('target_%d', j));
            %save([output_location_feature sprintf('train2_1regression2_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
            %save([output_location_feature sprintf('train2_1regression2_%d', j)], sprintf('input_%d', j), sprintf('target_%d', j));
            %save(sprintf('MATLAB/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/train2_1regression2_%d', i), sprintf('input_%d', i), sprintf('target_%d', i));
            %save(sprintf('MATLAB/NORMALIZE_MULTIPLE/REGRESSION1_DATASET2/train2_1regression2_%d', j), sprintf('input_%d', j), sprintf('target_%d', j));
            %save(sprintf('15_16/original_2/train_1regression_%d', i), sprintf('input_%d', i), sprintf('target_%d', i));
            %save(sprintf('15_16/original_2/train_1regression_%d', j), sprintf('input_%d', j), sprintf('target_%d', j));
            clear('input', 'target', sprintf('input_%d', i), sprintf('target_%d', i), sprintf('input_%d', j), sprintf('target_%d', j));
...
for i=1:18
    ...
    for j=1:18
        if(i~=j)
            ...
            input_all = ['input_', num2str(i), '= input(1:test_len(i),:);'];
            eval(input_all);
            target_all = ['target_', num2str(i), '= target(1:test_len(i),:);'];
            eval(target_all);
            input_all = ['input_', num2str(j), '= input(1+test_len(i):end,:);'];
            eval(input_all);
            target_all = ['target_', num2str(j), '= target(1+test_len(i):end,:);'];
            eval(target_all);
            
            
            save([output_location_target '/' sprintf('test_1regression2_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
            save([output_location_target '/' sprintf('test_1regression2_%d', i)], sprintf('input_%d', i), sprintf('target_%d', i));
            ...
            %save(sprintf('15_16/original_2/test_1regression_%d', i), sprintf('input_%d', i), sprintf('target_%d', i));
            %save(sprintf('15_16/original_2/test_1regression_%d', j), sprintf('input_%d', j), sprintf('target_%d', j));
            clear('input', 'target', sprintf('input_%d', i), sprintf('target_%d', i), sprintf('input_%d', j), sprintf('target_%d', j));
