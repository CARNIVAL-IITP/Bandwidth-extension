...
def mask_input(sig):
    if CONFIG.TASK.masking_strategy == 'unstructured':
        sig = np.reshape(sig, (-1, random.choice(CONFIG.TASK.unstructured_mask_chunk)))
        mask = np.ones(len(sig))
        mask[:int(random.choice(CONFIG.TASK.unstructed_mask_ratio) * len(mask))] = 0
...
class BlindTestLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # sig = sig[:, :(sig.shape[1] // self.p_size) * self.p_size]
        # sig = torch.tensor(sig).squeeze(0)
        # sig = sig.clone()
