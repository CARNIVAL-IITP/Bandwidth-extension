...
def test_predictor():
    predictor = Predictor(window_size=320, lstm_dim=512, lstm_layers=1, 
                          pred_lstm_type='GRU')
    fake_input = torch.zeros(16, 1, 160, 1)#.cuda()
    # x = np.ndarray([traj_lstm(fake_input)])
    print(predictor(fake_input).shape)
    exit()
...
class Predictor_traj_lstm(pl.LightningModule):  # mel
    def __init__(self, window_size=1536, sr=48000, lstm_dim=256, lstm_layers=3, 
                 n_mels=64):
        super(Predictor_traj_lstm, self).__init__()
        self.window_size = window_size
        self.hop_size = window_size // 2
        self.lstm_dim = lstm_dim
        self.n_mels = n_mels
        self.lstm_layers = lstm_layers
        # self.pred_lstm_type = pred_lstm_typ
        ...
        # self.lstm = nn.LSTM(input_size=self.n_mels, hidden_size=self.lstm_dim, bidirectional=False,
        #                     num_layers=self.lstm_layers, batch_first=True)
        # self.lstm = TrajLSTM(input_size=self.n_mels, hidden_size=self.lstm_dim, 
        #                      num_layers=self.lstm_layers, batch_first=True)
        self.traj_lstm = LT_LSTM(input_size=self.n_mels, hidden_size=self.lstm_dim, 
                             num_layers=self.lstm_layers, batch_first=True)
        self.expand_dim = nn.Linear(self.lstm_dim, self.n_mels)
        self.inv_mel = nn.Linear(self.n_mels, self.hop_size)
    ...
    def forward(self, x, state=None):  # B, 2, F, T
        ...
        if state is None:
            ...
            x, state = self.traj_lstm(x, (state[0], state[1]))
            # x, state = self.lstm(y, (state[0], state[1]))
        x = self.expand_dim(x)
        x = torch.abs(self.inv_mel(torch.exp(x)))
        x = x.permute(0, 2, 1)
        x = x.reshape(B, C, -1, T)
