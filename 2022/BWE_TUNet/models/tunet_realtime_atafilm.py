...
from models.trasnformer import TransformerBlock
...
class AFiLM(nn.Module):
    def __init__(self, block_size, input_dim, **kwargs):
        ...
        self.max_len = CONFIG.DATA.window_size // 128 #block_size
        self.transformer = TransformerBlock(num_layers = 4, embed_dim= input_dim, 
                                            num_heads=8, ff_dim=2048, maximum_position_encoding=self.max_len)
...
class Encoder(nn.Module):
    def __init__(self, max_len, kernel_sizes, strides, out_channels, tfilm, afilm, n_blocks):
        ...
        if self.afilm:
            b_size = max_len // (n_blocks * strides[0])
            self.afilm_d = AFiLM(block_size=b_size, input_dim=out_channels[0])
            b_size //= strides[1]
            self.afilm_d1 = AFiLM(block_size=b_size, input_dim=out_channels[1])
        
        self.downconv = CausalConv1d(in_channels = 1, out_channels = out_channels[0], kernel_size = kernel_sizes[0],
                                    stride = strides[0])
        self.downconv1 = CausalConv1d(in_channels = out_channels[0], out_channels = out_channels[1], 
                                    kernel_size = kernel_sizes[1], stride = strides[1])
        self.downconv2 = CausalConv1d(in_channels = out_channels[1], out_channels = out_channels[2], 
...
class Decoder(nn.Module):
    def __init__(self, in_len, kernel_sizes, strides, out_channels, tfilm, afilm, n_blocks):
        ...
        if self.afilm:
            in_len *= strides[2]
            self.afilm_u1 = AFiLM(block_size=in_len // n_blocks, input_dim=out_channels[1])
            in_len *= strides[1]
            self.afilm_u = AFiLM(block_size=in_len // n_blocks, input_dim=out_channels[0])
...
class BaseModel(pl.LightningModule):
    def __init__(self, train_dataset=None, val_dataset=None):
        ...
        self.stoi_loss = NegSTOILoss(sample_rate=CONFIG.DATA.sr)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        self.log('val_sisdr', self.valid_sisdr, on_step=False, on_epoch=True, logger=True, prog_bar=True,
                 sync_dist=True)
        self.log('val_stoi', self.valid_stoi, on_step=False, on_epoch=True, logger=True, prog_bar=True,
                 sync_dist=True)
        self.log('val_estoi', self.valid_estoi, on_step=False, on_epoch=True, logger=True, prog_bar=True,
                 sync_dist=True)
        if batch_idx == 0:
...
class TUNet_reatime_atafilm(BaseModel):
    def __init__(self, train_dataset, val_dataset):
        super(TUNet_reatime_atafilm, self).__init__(train_dataset, val_dataset)
        ...
        self.hparams['causal'] = CONFIG.MODEL.causal
        self.hparams['afilm'] = CONFIG.MODEL.afilm
